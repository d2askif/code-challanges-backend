...
describe("available dates with time window", () => {
  const mondayPref: preferredTimeWindow = {
    day: "monday",
    startTimeInMinutes: 600,
    endTimeInMinutes: 1200
  };
  const wednesdayPref: preferredTimeWindow = {
    day: "wednesday",
    startTimeInMinutes: 600,
  ...
  test("available dates will be given according to the weekdays preference", () => {
    ...
    const dateWithTimeWindows = mapAvailableDateToTimeWindows(
      preference,
      availableDate
    );
    ...
    expect(dateWithTimeWindows[0]).toEqual({
      date: "2019:10:14",
      bookingStartTime: moment(dateMonday, "YYYY:MM:DD")
        .minutes(mondayPref.startTimeInMinutes)
        .toDate(),
      bookingEndTime: moment(dateMonday, "YYYY:MM:DD")
    ...
    expect(dateWithTimeWindows[1]).toEqual({
      date: "2019:10:16",
      bookingStartTime: moment(dateWednesday, "YYYY:MM:DD")
        .minutes(wednesdayPref.startTimeInMinutes)
        .toDate(),
      bookingEndTime: moment(dateWednesday, "YYYY:MM:DD")
  ...
  test("available booking start time and booking end time should be with in the same date", () => {
    ...
    expect(dateWithTimeWindows.length).toBe(2);
    expect(
      dateWithTimeWindows[0].bookingEndTime.getDate() ===
        dateWithTimeWindows[0].bookingStartTime.getDate()
    ).toBe(true);

    expect(
      dateWithTimeWindows[1].bookingEndTime.getDate() ===
        dateWithTimeWindows[1].bookingStartTime.getDate()
    ).toBe(true);
  });
...
describe("generateTimeSlots", () => {
  test("generates time slots with correct interval", () => {
    ...
    expect(
      timeSlots[0].endTime.getTime() - timeSlots[0].startTime.getTime()
    ).toBe(intervalMinutes * 60 * 1000);
  });
  ...
  test("time slots lie with in the startTime and end Time", () => {
    ...
    expect(
      firstTimeSlot.startTime.getTime() === startTime.getTime()
    ).toBeTruthy();
...
describe("checkIfTimeSlotsOverLap", () => {
  ...
  it("when the time slots  do not not over lapp", () => {
    const generatedTimeSlot: ITimeSlot = {
      startTime: new Date("Wed, 16 October 2019 13:51:00"),
      endTime: new Date("Wed, 16 October 2019 13:55:00")
    };
    const isOverLap = checkIfTimeSlotsOverLap(bookedSlot, generatedTimeSlot);
